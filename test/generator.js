...
describe('#generateMessageBodies', () => {
  describe('Generate a message body', () => {
    let results;
    ...
    before(function() {
      ...
      results = queryElement(element, {element: "httpRequest"});
    });
  ...
  describe('Generate multiple message bodies', () => {
    ...
    describe('Second request', () => {
      it('‘messageBodySchema’ has not been mutated', () => {
        assert.deepEqual(results[1].content[0], {
          element: 'asset',
          meta: {
            classes: [
              'messageBodySchema'
      ...
      it('‘messageBody’ element equals to ‘asset’', () => {
        assert.strictEqual(results[1].content[1].element, 'asset');
      });

      it('‘messageBody’ content equals to a string', () => {
        assert.isString(results[1].content[1].content);
      });

      it('‘messageBody’ attributes has not been mutated', () => {
        assert.deepEqual(results[1].content[1].attributes, {
          contentType: 'application/json',
        });
      });

      it('‘messageBody’ meta has not been mutated', () => {
        assert.deepEqual(results[1].content[1].meta, {
          classes: [
            'messageBody'
          ],
        });
